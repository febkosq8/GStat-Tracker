...
function Repo() {
  ...
  const handleSubmit = async (e) => {
    if (repoUrl !== "") {
      ...
      repoMetadata = {
        ...repoMetadata,
        totalContributions: repoContributors.reduce(
          (acc, curr) => acc + curr.contributions,
          0
        ),
        contributors: repoContributors,
      };
      APIHandler.auditRepoQuery({
        timeStamp: new Date().toISOString(),
        response: {
          repoName: repoMetadata.name,
          repoUrl: repoMetadata.html_url,
          createdBy: repoMetadata.owner.login,
          createdAt: repoMetadata.created_at,
          lastCommitOn: repoMetadata.pushed_at,
          contributors: repoMetadata.contributors.map((user) => {
            return { name: user.login, commits: user.contributions };
          }),
          totalCommits: repoMetadata.totalContributions,
        },
      });
        ...
        {repoMetadata.html_url ? (
          <li className="list-group-item">
            <div class="box">
              ...
              <p>
                <strong>URL :</strong>{" "}
                <a href={repoMetadata.html_url}>{repoMetadata.html_url}</a>
              </p>
              <p>
                <strong>Created by :</strong>{" "}
                <a href={`/user?username=${repoMetadata.owner.login}`}>
              ...
              <p>
                ...
                {new Date(repoMetadata.pushed_at).toLocaleString()}
              </p>
              <p>
              ...
              <p>
                ...
                {repoMetadata.totalContributions}
              </p>
            </div>
          </li>
        ) : (
